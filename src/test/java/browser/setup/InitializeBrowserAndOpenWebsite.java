...
public class InitializeBrowserAndOpenWebsite {
	...
	public Properties loadProperties() {
		prop = new Properties();
		try {
			File file = new File("C:\\OpenCart-Selenium-Hybrid\\src\\test\\java\\browser\\setup\\config.properties");
	...
	public Properties loadDataProperties() {
		...
		try {
			File file = new File("C:\\OpenCart-Selenium-Hybrid\\src\\test\\java\\testdata\\testdata.properties");
			FileInputStream fis = new FileInputStream(file);
			dataProp.load(fis);
		} catch (Exception e) {
			e.printStackTrace();
